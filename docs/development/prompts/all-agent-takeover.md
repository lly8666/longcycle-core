...
3. 只有前一个 `H` 已验证为 `CLEAN`，才重读新 map/cursor并选择下一个 probe。第二、第三、第四个 probe 不能预排、不能并行，必须由前一个结果递归启发。
4. 完成 4 个完整循环后结束本次执行。若没有有效 `next_probe`、需要 seal/challenger/review、下一步需要当前缺失的能力、出现 L3/L4/边界问题、continuity 不安全，或剩余执行能力不足以安全完成下一次 `S/H`，立即提前停止；不得为了凑数发明 probe。
5. 前三个 probe 结束后的 `H` 是安全检查点，不是要求用户再发“继续”。同一 Agent 在 `CLEAN` 后自动进入下一循环。
