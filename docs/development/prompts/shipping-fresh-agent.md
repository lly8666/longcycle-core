...
先用简短进度消息报告：main/航运精确 head、continuity 结果及依据、六层目标、允许/禁止输入与路径、第一个 probe、三连跑目标、工具限制和 L3/L4。若为 `CLEAN`，不等待用户回复，顺序执行最多 3 个完整 probe 循环；每个循环都执行 `docs/development/prompts/all-agent-handoff.md` 并达到远端 `S -> cursor-only H -> CLEAN` 后，才自动进入下一个。
