...
对于行业 Memory Campaign Lead，专用提示词默认采用“一次聊天执行最多 3 个顺序 probe”的切片：每个 probe 独立 `S -> H -> CLEAN`，后一个只能由前一个完成后更新的地图选择。它是提高单次有效工作量的软目标，不是固定时长、完成度配额或预排三任务；没有安全下一步时提前停止。
